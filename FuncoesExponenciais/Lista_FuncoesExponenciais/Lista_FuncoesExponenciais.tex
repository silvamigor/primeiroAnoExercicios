\documentclass[a4paper,12pt]{article}

\input{../../Preambulo.tex}
\newcommand{\assunto}{FUNÇÕES EXPONENCIAIS}
\newcommand{\atividade}{lista} % prova ou lista
\input{../../Variaveis.tex}

\hypersetup{
    pdfauthor={\professor},
    pdftitle={\titulo \ -- \assunto \ -- \turma},
}

\title{\titulo \ -- \assunto \ -- \turma}
\author{\professor}
\date{\data}

\graphicspath{
    {../../FuncoesExponenciais/}
}

%************* INÍCIO DO DOCUMENTO *************
\begin{document}
    \subtitulolis
    
    \vspace{13pt}
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 1}
    \input{../FE_Exercicio1_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 2}
    \input{../FE_Exercicio2_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 3}
    \input{../FE_Exercicio3_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 4}
    \input{../FE_Exercicio4_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 5}
    \input{../FE_Exercicio5_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 6}
    \input{../FE_Exercicio6_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 7}
    \input{../FE_Exercicio7_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 8}
    \input{../FE_Exercicio8_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 9}
    \input{../FE_Exercicio9_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 10}
    \input{../FE_Exercicio10_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 11}
    \input{../FE_Exercicio11_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 12}
    \input{../FE_Exercicio12_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 13}
    \input{../FE_Exercicio13_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 14}
    \input{../FE_Exercicio14_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 15}
    \input{../FE_Exercicio15_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 16}
    \input{../FE_Exercicio16_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 17}
    \input{../FE_Exercicio17_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 18}
    \input{../FE_Exercicio18_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 19}
    \input{../FE_Exercicio19_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 20}
    \input{../FE_Exercicio20_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 21}
    \input{../FE_Exercicio21_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 22}
    \input{../FE_Exercicio22_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 23}
    \input{../FE_Exercicio23_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 24}
    \input{../FE_Exercicio24_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
    \phantomsection
    \addcontentsline{toc}{section}{Exercício 25}
    \input{../FE_Exercicio25_1}
    \noindent\rule{\linewidth}{1pt}  % linha horizontal fina
    
\end{document}
%*************** FINAL DO DOCUMENTO ***************
